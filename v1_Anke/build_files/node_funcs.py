...
def generate_random_positions_square(N, layer_range, radial_range):
    radius_outer = radial_range[1]
    radius_inner = radial_range[0]
    layer_start = layer_range[0]
    layer_end = layer_range[1]
    ...
    y = []
    while len(x)<N:
        x_try = (2*np.random.random()-1)*radius_outer
        z_try = (2*np.random.random()-1)*radius_outer
        if abs(x_try) > radius_inner or abs(z_try) > radius_inner:
            x.append(x_try)
            z.append(z_try)
            y.append((layer_end - layer_start) * np.random.random() + layer_start)
...
def get_filter_temporal_params(N, X_grids, Y_grids, model):
    ...
    basepath = 'build_files/biophys_props/lgn_fitted_models/'

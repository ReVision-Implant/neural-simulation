    ...
    for amplitude in amplitudes:
        for electrode in electrodes:
            for network in networks:
                nodes_dirs.append('networks_' + density + '/network' + network + '/v1_nodes.h5')
                spikes_dirs.append('exp' + exp + '/output/'+ electrode + '/' + stim_type + '/' + amplitude + '/0' + electrode + stim_type + '_' + amplitude + '_' + network + '/spikes.csv')
                spikes_bg_dirs.append('exp' + exp + '/output/bkg/bkg_' + network + '/spikes.csv')
            electrodes_dirs.append('../bio_components/stimulations/exp' + exp + '/elec_loc/' + electrode + '.csv')

    return dict(nodes_dirs=nodes_dirs, spikes_dirs=spikes_dirs, spikes_bg_dirs=spikes_bg_dirs, electrodes_dirs=electrodes_dirs, radius=radius, v1=True)
...
class Plotter():
    ...
    def get_grid(self):
        ...
        grid = np.zeros((self.radius*2,self.radius*2))

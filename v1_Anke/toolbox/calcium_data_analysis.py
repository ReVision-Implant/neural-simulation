...
plt.tight_layout()
# todo PCA toepassen op de vectoren in de 13-dimensionale ruimte van de 13 verschillende clusters om zo een significant verschillend aantal clusters te ontdekken
# todo first group the neurons in bins to account for some Z-drift?

variances = []
for slice in list_directions20:
    activity_matrix = [] # N x 13 with N the number of ROIs and 13 the number of current directions
    for pattern in range(slice.nb_patterns):
        # Take all fluorescence values per stimulation pattern, but only for cells that are active for at least 1 pattern
        activity_matrix.append([slice.cell_data[cell]['peaks'][pattern] for cell in slice.cell_data.keys() if 1 in [slice.cell_data[cell]['active'][patt] for patt in range(slice.nb_patterns)]])
...
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.title('PCA on neural activity for 13 directions', fontsize=20)
plt.tight_layout()
...
plt.gca().get_yaxis().set_major_formatter(FuncFormatter(lambda x, p: format(int(x*150), ','))) # scaling to get standard deviation in µm. 150 is distance between the two needles # todo use round numbers?
plt.yticks(fontsize=15)

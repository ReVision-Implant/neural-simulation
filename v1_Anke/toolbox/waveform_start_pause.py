...
def CreateBlockWaveform(n_pulses, pause_duration, phase_1_expr, amp_1_expr, T_1_expr, phase_2_expr, amp_2_expr, T_2_expr, save_name=None):
    """Create a block waveform using the CreateWaveform class. Except for n_pulses and pause_duration, all arguments should be lambda expressions of n (units: ms).
    ...
    :type n_pulses: int
    :param pause_duration: duration of the pause before the start of the waveform in ms
    ...
    t_start = pause_duration  # Start time after the pause
    ...
    for i in range(n_pulses):
        ...
        piecewise_temp1 = [t_start, lambda t:0]  # Pause before the pulse
        piecewise_temp2 = [t_start+phase_1, lambda t, amp_1=amp_1:amp_1]
        piecewise_temp3 = [t_start+phase_1+T_1, lambda t:0]
        piecewise_temp4 = [t_start+phase_1+T_1+phase_2, lambda t, amp_2=amp_2:amp_2]
        piecewise_temp5 = [t_start+phase_1+T_1+phase_2+T_2, lambda t:0]
        piecewise = np.vstack((piecewise, piecewise_temp1, piecewise_temp2, piecewise_temp3, piecewise_temp4, piecewise_temp5))  # Add pulse i
...
if __name__ == '__main__':
    ...
    CreateBlockWaveform(
        ...
        T_1_expr = lambda n:0,
        phase_2_expr = lambda n:1.6,
        amp_2_expr = lambda n:0.125,
        T_2_expr = lambda n:4,
        save_name = "/components/stimulation/waveforms/waveform_4_-8_5ms.csv"
    )